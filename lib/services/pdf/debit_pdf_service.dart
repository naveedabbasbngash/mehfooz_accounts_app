...
class DebitPdfService extends BasePdfService {
  ...
  Future<File> render({
    ...
    // ------------------------------------------------------------------
    // 🔥 FILTER: remove rows where ALL debit-only values are 0
    // ------------------------------------------------------------------
  ...
  pw.Widget _buildTable({
    ...
    pw.Widget debitCell(int raw) {
      final debitOnly = raw < 0 ? -raw : 0;
      final textColor = debitOnly > 0 ? negativeRed : black;
    ...
    pw.Widget totalsValueCell(int value) {
      final textColor = value > 0 ? negativeRed : deepBlue;
